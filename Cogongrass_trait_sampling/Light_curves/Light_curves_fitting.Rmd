...
model{
 ...
 precision ~ dgamma(0.1, 5) ## Guessing
}
"
...
init <- list()
for(i in 1:nchain){
  init[[i]] <- list(m = 21,  min_conductance = 0.0001, D_0 = 0.016, precision = 0.01)
}
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c("m","min_conductance", "precision", "D_0"), #"D_0",
                                n.iter = n.iter)
...
if(is.na(burnin) | rlang::is_empty(burnin)){
  ...
  file_name <- paste("/Users/tess/Documents/work/SERDP_Project/Cogongrass_trait_sampling/Jags_output/", date_stamp, ".", "Stomatal_slope_of",".",leaf_name,".", did_it_converge, ".","JAGS_run_humidity_fix.Rdata", sep = "")
...
```{r}
jags_list <- list.files("/Users/tess/Documents/work/SERDP_Project/Cogongrass_trait_sampling/Jags_output/")
jags_list <- jags_list[grep("20200303.*", jags_list)]
jags_list <- jags_list[grep("*passed*", jags_list)]
jags_list <- jags_list[grep("*humidity_fix*", jags_list)]
...
load(paste("/Users/tess/Documents/work/SERDP_Project/Cogongrass_trait_sampling/Jags_output/", jags_list[i], sep = ""))

name <- gsub("20200303.Stomatal_slope_of.", "", jags_list[i])
name <- gsub(".convergence_passed_GBR_test.JAGS_run_humidity_fix.Rdata", "", name)

jags.mat <- as.matrix(jags.burn)

slope <- quantile(jags.mat[,2], c(0.025, 0.5, 0.975))
intercept <- quantile(jags.mat[,3], c(0.025, 0.5, 0.975))
#D_0_quantiles <- quantile(jags.mat[,1], c(0.025, 0.5, 0.975))
#D_0_quantiles <- D_0_quantiles*100

plotting_data <- full_curve
### Regenerate X with new D_0 value
#X <- A/(Ca*(1+(vpd/D_0)))
#plotting_data$X_corrected <- plotting_data$A/(plotting_data$Ca*(1 + (plotting_data$vpd/D_0_quantiles[2])))



series <- seq(from = min(na.omit(plotting_data$X)), to = max(na.omit(plotting_data$X)) ,length.out = length(na.omit(plotting_data$X)))
  
upper_line <- (series*slope[1]) + intercept[1]
mean_line <-  (series*slope[2]) + intercept[2]
lower_line <- (series*slope[3]) + intercept[3]


ggplot(data = na.omit(plotting_data)) +
  geom_point( aes(x = X, y = gs, color = Name)) +
  geom_line( aes(y = upper_line, x = series), color = "black", size = 0.5 ) +
  geom_line(aes(y = mean_line, x = series), color = "black", size = 1 )+
  geom_line(aes(y = lower_line, x = series), color = "black", size = 1 )+
  ylab("Stomatal Conductance")+
  xlab("VPD corrected photosynthesis")+
