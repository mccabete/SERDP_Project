...
init <- list()
for(i in 1:nchain){
  init[[i]] <- list(m = 0.5,  min_conductance = 0.001) #, D_0 = 0.016)
}
...
for ( i in seq_along(leaves)){
  ...
  run_stomatal_slope_fit(temp_data = temp_data, leaf_name = leaves[i], n.iter = 60000 )
...
jags_list <- jags_list[grep("2020*", jags_list)]
...
load(paste("/Users/tess/Documents/work/SERDP_Project/Cogongrass_trait_sampling/Jags_output/", jags_list[1], sep = ""))
...
series <- seq(from = min(first_curve_only$X), to = max(first_curve_only$X) ,length.out= 42)
...
#sanity_check <- glm(first_curve_only$gs ~ first_curve_only$X)
