...
model{
 ...
 precision ~ dgamma(0.8, 2) ## Guessing
}
"
...
data <- list(num_points = length(temp_data$gs), 
             ...
             stomatal_slope_average = 2) #stomatal_slope_average)
...
if(is.na(burnin) | rlang::is_empty(burnin)){
  ...
  file_name <- paste("/Users/tess/Documents/work/SERDP_Project/Cogongrass_trait_sampling/Jags_output/", date_stamp, ".", "Stomatal_slope_of",".",leaf_name,".", did_it_converge, ".","JAGS_run_test_of_d_0.Rdata", sep = "")
...
jags_list <- jags_list[grep("20200304.*", jags_list)]
jags_list <- jags_list[grep("*passed*", jags_list)]
jags_list <- jags_list[grep("*test_of_d_0*", jags_list)]
...
name <- gsub("20200304.Stomatal_slope_of.", "", jags_list[i])
name <- gsub(".convergence_passed_GBR_test.JAGS_run_run_test_of_d_0.Rdata", "", name)
