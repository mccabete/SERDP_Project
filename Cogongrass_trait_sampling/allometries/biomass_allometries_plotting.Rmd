...
plot(density(na.omit(below2aboveground_amb)), main = "Ratio of belowground biomass to aboveground", col = "blue")
...
```{r}
Drew_cogongrass <- read_excel("/Users/tess/Documents/work/SERDP_Project/Cogongrass_trait_sampling/SLA/CogonComplete-2.xls")
...
## AV hight
tess_height <- rep(NA, length(biomass[1,]))

for(i in 1:length(biomass[1,]) ){
  av_tmp <- strsplit(as.character(biomass$Cogongrass.Hieghts.m..Subsample.[i]), split = ",")
  av_tmp <- as.numeric(unlist(av_tmp))
  av_tmp <- mean(av_tmp)
  tess_height[i] <- av_tmp
}
